...
const Avatar = ({ size }) => {
    ...
    return (
        <div className={`w-${width} h-${height} rounded-full overflow-hidden mg-auto`}>
            <img
...
const Hsction8 = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:8090/testreview/read")
            .then((res) => {
                setReviews(res.data);
            })
            .catch((err) => {
                                    ...
                                    <Carousel responsive={responsive} autoPlay autoPlaySpeed={2000} infinite>
                                        {reviews.map((review) => (
                                            <div className='cardBlock-1' key={review._id}>
                                                <div className="m-3">
                                                    <a href={'/reviews/' + review._id} style={{ textDecoration: "none", color: "#171617" }}>
                                                        <div className="card" id='cardv'>
                                                            
                                                            <div className="card-body" id="tit-card" style={{ height: "300px", overflow: "hidden" }}>
                                                                
                                                            <Avatar size="lg" />
                                                                <p style={{ marginTop: "5px", fontWeight: "bold", fontSize: "18px" }}> {review.fullName}</p>
                                                                ...
                                                                <div className="row">
                                                                    <div className="col">
                                                                        <p style={{ marginTop: "10px", fontSize: "16px", color: "#696969" }}> : {review.destination}</p>
                                                                        <p style={{ marginTop: "10px", color: "#fcb900", fontSize: "18px" }}>
